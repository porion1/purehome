...
// 6 lines - Prevents thundering herd, skips non-retryable errors
// ============================================
const retry = async (fn, retries = 3, delay = 100) => {
    for (let i = 0; i < retries; i++) {
        try { return await fn(); }
            ...
            // Don't retry fraud blocks (403) or validation errors (400)
...
// 20 lines - All payment operations in one place
// ============================================
...
const paymentClient = {
    ...
    getSystemHealth: async (authToken) => {
        return retry(async () => {
